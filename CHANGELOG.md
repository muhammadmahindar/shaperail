...
All notable changes to SteelAPI will be documented in this file.
...
- Fixed crate publish manifests so internal Steel dependencies include version requirements and package cleanly for crates.io
- Aligned the install script with GitHub release archive names and the real repository coordinates
- Added `steel serve --check` plus CLI smoke coverage for scaffolded project validation and compile checks

## [0.2.0] - 2026-03-09

### Added

- **Core Types** — `ResourceDefinition`, `FieldType`, `FieldSchema`, `EndpointSpec`, `AuthRule`, `RelationSpec`, `IndexSpec`, `CacheSpec`, `SteelError` with standardized error responses
...
- **CLI** — `steel init`, `generate`, `serve`, `build`, `validate`, `test`, `migrate`, `seed`, `export openapi`, `export sdk`, `doctor`, `routes`, `jobs:status`
...
[0.2.1]: https://github.com/muhammadmahindar/steel-api/releases/tag/v0.2.1
[0.2.0]: https://github.com/muhammadmahindar/steel-api/releases/tag/v0.2.0
