...
fn steel() -> Command {
    Command::cargo_bin("steel").unwrap()
}
...
#[test]
fn version_flag() {
    steel().arg("--version").assert().success();
}
...
fn init_creates_project_structure() {
    ...
    steel()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Created SteelAPI project"));

    // Check directory structure
    assert!(project_dir.join("steel.config.yaml").exists());
    ...
    let config = std::fs::read_to_string(project_dir.join("steel.config.yaml")).unwrap();
...
fn doctor_runs() {
    steel()
        .args(["doctor"])
        .assert()
        .stdout(predicate::str::contains("SteelAPI Doctor"));
}
...
fn setup_project_dir(tmp: &TempDir) {
    ...
    std::fs::write(tmp.path().join("steel.config.yaml"), config).unwrap();
    std::fs::create_dir(tmp.path().join("resources")).unwrap();
    std::fs::copy(
        workspace_root().join("resources/users.yaml"),
        tmp.path().join("resources/users.yaml"),
