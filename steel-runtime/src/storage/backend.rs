...
impl From<StorageError> for steel_core::SteelError {
    fn from(err: StorageError) -> Self {
        match err {
            StorageError::NotFound(_) => steel_core::SteelError::NotFound,
            ...
            StorageError::Backend(msg) => steel_core::SteelError::Internal(msg),
        }
    }
}

/// Storage backend selected via `STEEL_STORAGE_BACKEND` env var.
///
...
impl StorageBackend {
    /// Create a storage backend from the `STEEL_STORAGE_BACKEND` env var.
    ///
    /// Supported values: `local`, `s3`, `gcs`, `azure`.
    /// Defaults to `local` if not set.
    pub fn from_env() -> Result<Self, StorageError> {
        let backend =
            std::env::var("STEEL_STORAGE_BACKEND").unwrap_or_else(|_| "local".to_string());
