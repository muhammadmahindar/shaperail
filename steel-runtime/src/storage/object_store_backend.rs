...
/// - `AWS_DEFAULT_REGION` or `STEEL_STORAGE_REGION`
/// - `STEEL_STORAGE_BUCKET`
...
impl S3Storage {
    ...
    pub fn from_env() -> Result<Self, StorageError> {
        let bucket = std::env::var("STEEL_STORAGE_BUCKET").map_err(|_| {
            StorageError::Backend("STEEL_STORAGE_BUCKET env var required for S3".to_string())
        })?;
        let region = std::env::var("STEEL_STORAGE_REGION")
...
impl GcsStorage {
    ...
    pub fn from_env() -> Result<Self, StorageError> {
        let bucket = std::env::var("STEEL_STORAGE_BUCKET").map_err(|_| {
            StorageError::Backend("STEEL_STORAGE_BUCKET env var required for GCS".to_string())
        })?;
...
/// - `STEEL_STORAGE_BUCKET` (container name)
...
impl AzureStorage {
    /// Create from environment variables.
    pub fn from_env() -> Result<Self, StorageError> {
        let container = std::env::var("STEEL_STORAGE_BUCKET").map_err(|_| {
            StorageError::Backend("STEEL_STORAGE_BUCKET env var required for Azure".to_string())
        })?;
