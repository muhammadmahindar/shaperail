...
pub fn enforce(
    ...
    match rule {
        ...
        AuthRule::Owner => {
            ...
            if user.is_none() {
                return Err(SteelError::Unauthorized);
            }
            Ok(())
        }
        AuthRule::Roles(roles) => {
            let authenticated = user.ok_or(SteelError::Unauthorized)?;
...
pub fn check_owner(user: &AuthenticatedUser, record: &serde_json::Value) -> Result<(), SteelError> {
    let created_by = record.get("created_by").and_then(|v| v.as_str());

    match created_by {
        Some(owner_id) if owner_id == user.id => Ok(()),
        _ => Err(SteelError::Forbidden),
    }
}
