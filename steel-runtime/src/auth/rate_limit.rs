...
impl RateLimiter {
    ...
    /// or `Err(SteelError::RateLimited)` if the limit is exceeded.
    pub async fn check(&self, key: &str) -> Result<u64, SteelError> {
        let redis_key = format!("steel:ratelimit:{key}");
        ...
        let count: u64 = script
            ...
            .await
            .map_err(|e| SteelError::Internal(format!("Redis rate limit error: {e}")))?;

        if count > self.config.max_requests {
            return Err(SteelError::RateLimited);
        }
