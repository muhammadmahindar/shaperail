...
fn extract_auth(req: &HttpRequest) -> Result<AuthenticatedUser, SteelError> {
    // Try JWT first
    if let Some(auth_header) = req.headers().get("Authorization") {
        let header_str = auth_header.to_str().map_err(|_| SteelError::Unauthorized)?;
        if let Some(token) = header_str.strip_prefix("Bearer ") {
            let jwt_config = req
                .app_data::<web::Data<Arc<JwtConfig>>>()
                .ok_or(SteelError::Internal("JWT not configured".to_string()))?;
            let claims = jwt_config
                .decode(token)
                .map_err(|_| SteelError::Unauthorized)?;
