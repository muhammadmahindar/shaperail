...
use steel_core::{EndpointSpec, FieldError, ResourceDefinition, SteelError};
...
) -> Result<Option<AuthenticatedUser>, SteelError> {
...
) -> Result<HttpResponse, SteelError> {
...
) -> Result<serde_json::Value, SteelError> {
    ...
    if let (Some(ref cache), Some(ref cache_spec)) = (&state.cache, &endpoint.cache) {
        if !should_bypass_cache(&req, user.as_ref()) {
            ...
            let body = serde_json::to_string(&result)
                .map_err(|e| SteelError::Internal(format!("JSON serialization error: {e}")))?;
            cache.set(&cache_key, &body, cache_spec.ttl).await;
            return Ok(HttpResponse::Ok()
                .content_type("application/json")
                .insert_header(("X-Cache", "MISS"))
...
pub async fn handle_bulk_create(
    ...
    let items = body.as_array().ok_or_else(|| {
        SteelError::Validation(vec![FieldError {
    ...
    if items.len() > 500 {
        return Err(SteelError::Validation(vec![FieldError {
            field: "body".to_string(),
...
fn parse_uuid(s: &str) -> Result<uuid::Uuid, SteelError> {
...
fn extract_input(
    ...
    resource: &ResourceDefinition,
    endpoint: &EndpointSpec,
) -> Result<serde_json::Map<String, serde_json::Value>, SteelError> {
...
mod tests {
    ...
    use steel_core::{FieldSchema, FieldType, HttpMethod};
