...
impl JobPriority {
    ...
    pub fn queue_key(&self) -> &'static str {
        match self {
            JobPriority::Critical => "steel:jobs:queue:critical",
            JobPriority::High => "steel:jobs:queue:high",
            JobPriority::Normal => "steel:jobs:queue:normal",
            JobPriority::Low => "steel:jobs:queue:low",
        }
    }
}
...
impl JobQueue {
    ...
    pub async fn enqueue_with_options(
        ...
        let envelope_json = serde_json::to_string(&envelope)
            .map_err(|e| SteelError::Internal(format!("Failed to serialize job: {e}")))?;
        ...
        // Store job metadata
        let meta_key = format!("steel:jobs:meta:{id}");
        redis::cmd("HSET")
            .arg(&meta_key)
            .arg("id")
            .arg(&id)
            ...
            .await
            .map_err(|e| SteelError::Internal(format!("Failed to store job metadata: {e}")))?;
        ...
        conn.rpush::<_, _, ()>(priority.queue_key(), &envelope_json)
            .await
            .map_err(|e| SteelError::Internal(format!("Failed to enqueue job: {e}")))?;
    ...
    pub async fn get_status(&self, job_id: &str) -> Result<JobInfo, SteelError> {
        let mut conn = self
            .pool
            .get()
            .await
        ...
        let meta_key = format!("steel:jobs:meta:{job_id}");
        let values: Vec<String> = redis::cmd("HGETALL")
            .arg(&meta_key)
            .query_async(&mut *conn)
            .await
            .map_err(|e| SteelError::Internal(format!("Failed to get job status: {e}")))?;

        if values.is_empty() {
            return Err(SteelError::NotFound);
        }
    ...
    pub(crate) async fn update_status(
        ...
        cmd.query_async::<()>(&mut *conn)
            .await
            .map_err(|e| SteelError::Internal(format!("Failed to update job status: {e}")))?;
    ...
    pub(crate) async fn move_to_dead_letter(
        ...
        let dead_json = serde_json::to_string(&dead_entry)
            .map_err(|e| SteelError::Internal(format!("Failed to serialize dead letter: {e}")))?;

        conn.rpush::<_, _, ()>("steel:jobs:dead", &dead_json)
            .await
            .map_err(|e| {
                SteelError::Internal(format!("Failed to push to dead letter queue: {e}"))
            })?;
    ...
    pub(crate) async fn requeue_for_retry(
        ...
        conn.rpush::<_, _, ()>(envelope.priority.queue_key(), &envelope_json)
            .await
            .map_err(|e| SteelError::Internal(format!("Failed to requeue job: {e}")))?;
...
mod tests {
    ...
    fn priority_queue_keys() {
        assert_eq!(
            ...
            "steel:jobs:queue:critical"
        );
        assert_eq!(JobPriority::High.queue_key(), "steel:jobs:queue:high");
        assert_eq!(JobPriority::Normal.queue_key(), "steel:jobs:queue:normal");
        assert_eq!(JobPriority::Low.queue_key(), "steel:jobs:queue:low");
    }
