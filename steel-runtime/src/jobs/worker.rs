...
pub type JobHandler = Arc<
    dyn Fn(serde_json::Value) -> Pin<Box<dyn Future<Output = Result<(), SteelError>> + Send>>
        + Send
        + Sync,
>;
...
impl Worker {
    ...
    pub async fn poll_once(&self) -> Result<bool, SteelError> {
    ...
    async fn dequeue(&self, queue_key: &str) -> Result<Option<JobEnvelope>, SteelError> {
        let mut conn = self
            .queue
            .pool()
            .get()
        ...
        let result: Option<String> = conn
            .lpop(queue_key, None)
            .await
            .map_err(|e| SteelError::Internal(format!("Failed to dequeue job: {e}")))?;

        match result {
            Some(json) => {
                let envelope: JobEnvelope = serde_json::from_str(&json)
                    .map_err(|e| SteelError::Internal(format!("Failed to deserialize job: {e}")))?;
                Ok(Some(envelope))
            }
            None => Ok(None),
        }
    ...
    async fn handle_failure(&self, envelope: JobEnvelope, error: &str) -> Result<(), SteelError> {
    ...
    fn registry_lookup() {
        ...
        handlers.insert(
            ...
            Arc::new(|_payload: serde_json::Value| {
                Box::pin(async { Ok(()) })
                    as Pin<Box<dyn Future<Output = Result<(), SteelError>> + Send>>
            }) as JobHandler,
        );
