...
///
/// Cache keys follow the pattern: `steel:<resource>:<endpoint>:<query_hash>:<user_role>`
...
impl RedisCache {
    ...
    ///
    /// Format: `steel:<resource>:<endpoint>:<query_hash>:<role>`
    pub fn build_key(
        resource: &str,
        endpoint: &str,
        ...
        format!("steel:{resource}:{endpoint}:{query_hash}:{user_role}")
    }
    ...
    pub async fn invalidate_resource(&self, resource: &str) {
        ...
        let pattern = format!("steel:{resource}:*");
...
mod tests {
    ...
    fn build_key_format() {
        ...
        assert!(key.starts_with("steel:users:list:"));
