...
mod handler_unit_tests {
    ...
    use steel_core::*;
    use steel_runtime::handlers::response;
    ...
    fn validation_missing_required_returns_422() {
        ...
        let result = steel_runtime::handlers::validate::validate_input(&data, &resource);
        assert!(result.is_err());
        if let Err(SteelError::Validation(errors)) = result {
            assert!(errors.iter().any(|e| e.field == "name"));
            // SteelError::Validation maps to 422
            assert_eq!(
                SteelError::Validation(errors).status(),
    ...
    #[test]
    fn error_401_shape() {
        let err = SteelError::Unauthorized;
    ...
    fn error_422_shape_with_details() {
        ...
        let err = SteelError::Validation(errors);
        assert_eq!(
            err.status(),
            actix_web::http::StatusCode::UNPROCESSABLE_ENTITY
        );
    ...
    fn invalid_uuid_in_path_returns_422() {
        let err = SteelError::Validation(vec![FieldError {
            field: "id".to_string(),
...
#[cfg(test)]
mod auth_tests {
    use steel_core::{AuthRule, SteelError};
    use steel_runtime::auth::extractor::AuthenticatedUser;
    use steel_runtime::auth::jwt::JwtConfig;
    use steel_runtime::auth::rbac;
    ...
    fn rbac_401_no_token() {
        ...
        assert!(matches!(result, Err(SteelError::Unauthorized)));
    }
    ...
    fn rbac_403_wrong_role() {
        ...
        assert!(matches!(result, Err(SteelError::Forbidden)));
    }
    ...
    fn api_key_store_insert_and_lookup() {
        let mut store = steel_runtime::auth::api_key::ApiKeyStore::new();
        store.insert(
    ...
    fn api_key_store_invalid_key() {
        let store = steel_runtime::auth::api_key::ApiKeyStore::new();
    ...
    fn token_pair_serializes() {
        let pair = steel_runtime::auth::TokenPair {
    ...
    fn rate_limit_config_defaults() {
        let cfg = steel_runtime::auth::RateLimitConfig::default();
    ...
    fn rate_limit_key_generation() {
        let key_ip = steel_runtime::auth::RateLimiter::key_for("1.2.3.4", None);
        assert_eq!(key_ip, "ip:1.2.3.4");

        let key_user = steel_runtime::auth::RateLimiter::key_for("1.2.3.4", Some("u1"));
...
mod cache_tests {
    ...
    use steel_runtime::cache::RedisCache;
    ...
    fn cache_key_format_matches_spec() {
        ...
        // Format: steel:<resource>:<endpoint>:<hash>:<role>
        let parts: Vec<&str> = key.split(':').collect();
        assert_eq!(parts[0], "steel");
    ...
    fn cache_key_empty_params() {
        ...
        assert!(key.starts_with("steel:orders:get:"));
        assert!(key.ends_with(":anonymous"));
    }

    #[test]
    fn cache_pool_creation() {
        let pool = steel_runtime::cache::create_redis_pool("redis://localhost:6379");
        assert!(pool.is_ok());
    }
}
