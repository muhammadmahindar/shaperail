    ...
    runtime.block_on(async move {
        ...
        let pool = sqlx::PgPool::connect(&database_url).await.map_err(|error| {
            io::Error::other(format!(
                "Failed to connect to DATABASE_URL during build-time SQL verification: {error}. Run `docker compose up -d` in examples/incident-platform before building."
            ))
        })?;
...
fn database_url_from_env_or_dotenv() -> Result<Option<String>, Box<dyn std::error::Error>> {
    ...
    for dotenv_path in [".env", ".env.example"] {
        let iter = match dotenvy::from_path_iter(dotenv_path) {
            Ok(iter) => iter,
            Err(_) => continue,
        };
